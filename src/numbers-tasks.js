...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  ...
  const result = Math.abs((xSquare + ySquare) ** (1 / 2));
  return result;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const square1 = (x1 ** 2 + y1 ** 2) ** (1 / 2);
  const square2 = (x2 ** 2 + y2 ** 2) ** (1 / 2);
  const down = square1 * square2;
  const cos = up / down;
...
function roundToPowerOfTen(num, pow) {
  const degree = 10 ** pow;
  const result = Math.round(num / degree);
...
function isSafeInteger(number) {
  const a = Number.isSafeInteger(number);
  return a;
}
...
function getCountOfOddNumbers(number) {
  const int = number / 2;
  if (number < 0) return Math.abs(Math.floor(int));
